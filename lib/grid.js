...
function parseTables(html) {
  ...
  while ((m = tableRe.exec(html))) {
    ...
    while ((t = trRe.exec(inner))) {
      ...
      // ("No results found.") — seen live on girls with no service rows.
      if (row.cells.length === 1 && (row.cells[0].colspan > 1 || /^no results found\.?$/i.test(row.cells[0].text) || /\bempty\b/.test(row.cells[0].attrs.class || ''))) continue;
      rows.push(row);
    }
    ...
    // the grid's own pager: hrefs of ul.pagination after the table (before
    // the next table). A caller pages by following THESE (never by guessing
    // parameter names — Yii namespaces them when several grids share a page).
    ...
    const pagerHrefs = pg ? [...new Set(hrefsOf(pg[1]))] : [];
    out.push({ id, pos, headers, keys, rows, summaryRow, summary, toggleParam, pagerHrefs });
  }
  return out;
}
