 ...
 * @param {Object<string,{onRecord:number,earnable:number,hours?:number,legacyMode?:string,freshFrom?:string,freshHours?:number}>|null} [p.baseline]
 ...
 *     stars stand, and the Pathfinder hours they needed STAY counted: a fixed
 *     credit of exactly what the stars needed at baseline (never more than
 *     the girl's Pathfinder total). Every counted hour after that goes toward
 *     her next star; Pathfinder hours beyond the credit never count.
 *   - legacyMode 'fresh' (a leader's call per girl and level, e.g. a star
 *     awarded early, before a program year): every star on record at
 *     baseline stands and the level starts over at `freshFrom` — only its
 *     hours dated on or after that day (`freshHours`) count, nothing carries
 *     in. Expected = stars at baseline + what those hours earn; leftovers
 *     still carry on up.
 *   - otherwise legacy, 'separate' (the default): paper-era stars the ledger
 *     never held, added on top of what the hours earn.
 * The Pathfinder credit never exceeds the covered stars' worth, so counted
 * hours that DROP below the baseline still surface as a conflict.
 */
function computeStarChain({ hoursByLevel = {}, onRecord = {}, baseline = null, pathfinderHundredths = 0, rates = RATE_HUNDREDTHS } = {}) {
  ...
  for (const level of STAR_LEVELS) {
    ...
    const pathfinderExplained = level === 'Tenderheart' && extras && pathfinder
      ...
      : 0;
    const legacyMode = base && base.legacyMode === 'fresh' ? 'fresh' : 'separate';
    const fresh = legacyMode === 'fresh';
    const covered = fresh ? 0 : pathfinderExplained;
    // Pathfinder: a FIXED credit — exactly what the covered stars needed at
    // baseline — so every later counted hour goes toward the next star.
    const neededAtBaseline = Math.max(0, (baseEarnable + covered) * rate - baseAvailable);
    const credit = covered ? Math.min(pathfinder, covered * rate, neededAtBaseline) : 0;
    // fresh start: this level's hours since freshFrom only, nothing carried in
    const carryIn = fresh ? 0 : carry;
    const freshHours = fresh ? Math.max(0, int(base.freshHours)) : null;
    ...
    // stars that stand beyond what the counted hours earn: every baseline star
    // on a fresh start; otherwise the extras nothing above explains (paper-era)
    ...
    levels.push({
      ...
      legacyMode,
      credit,
      pathfinderCredit: credit,
      freshFrom: fresh ? (base.freshFrom || null) : null,
      freshHours,
      newStars: proposed,
      conflict,
      toNext: { hundredths: rate - carryOut, pct: Math.floor((carryOut * 100) / rate) },
      display: { hours: formatHundredths(hours), carryIn: formatHundredths(carryIn), carryOut: formatHundredths(carryOut) },
    });
    carry = carryOut;
  }
