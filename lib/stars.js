 ...
 *   - legacyMode 'hours' (a leader's call per girl and level): the extra
 *     stars count AGAINST her hours — a credit that fills the level up to
 *     them and shrinks to 0 as counted hours catch up, so later hours earn
 *     those stars back before the next one.
 *   - otherwise legacy, 'separate' (the default): paper-era stars the ledger
 *     never held, added on top of what the hours earn.
 * Neither credit exceeds the covered stars' worth, so counted hours that
...
function computeStarChain({ hoursByLevel = {}, onRecord = {}, baseline = null, pathfinderHundredths = 0, rates = RATE_HUNDREDTHS } = {}) {
  ...
  for (const level of STAR_LEVELS) {
    ...
    // 'hours': a credit that fills up to the covered stars and shrinks as
    // counted hours catch up (they earn those stars back first).
    const neededAtBaseline = Math.max(0, (baseEarnable + covered) * rate - baseAvailable);
    const credit = !covered ? 0
      : legacyMode === 'hours'
        ? Math.min(covered * rate, Math.max(0, (baseEarnable + covered) * rate - raw))
        : Math.min(pathfinder, covered * rate, neededAtBaseline);
