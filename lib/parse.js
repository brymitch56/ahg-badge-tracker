...
/** #badge-select → [{ awardId, name, imageSlug, levelGroup }] */
function parseBadgeSelect(html) {
  ...
  while ((m = re.exec(block))) {
    ...
    out.push({
      awardId: value,
      name: bar >= 0 ? text.slice(0, bar).trim() : text,
...
function parseFragment(html, ctx = {}) {
  ...
  if (levelIds.size === 0) warnings.push('no level id (le…) found in fragment');
...
module.exports = {
  RE, tokenize, parseAttrs, parseBadgeSelect, parseYouthSelect, parseLevelSelect, findLevelIds,
  classify, parseFragment, scrubPersonal, containsYouthId,
};
