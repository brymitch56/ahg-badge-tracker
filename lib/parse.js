 ...
 * ad… records. Defensive: absent fields come back null, unknown markup is
 * skipped. Output may contain an ad… record id — tracker.db only.
 */
function parseStandardState(html, { awardId = null } = {}) {
  ...
  while ((m = inputRe.exec(html))) {
    ...
    let f;
    if ((f = /^checkbox-([a-z0-9]{12})$/.exec(name)) && f[1] !== awardId) {
      item(f[1]).checked = 'checked' in attrs || /^(checked|true|1)$/i.test(attrs.checked || '');
    } else if ((f = /^date-([a-z0-9]{12})$/.exec(name)) && f[1] !== awardId) {
      const v = (attrs.value || '').trim();
      if (v) item(f[1]).date = v;
    } else if ((f = /^comment-([a-z0-9]{12})$/.exec(name)) && f[1] !== awardId) {
      const v = m[1].toLowerCase() === 'textarea' ? decodeHtml(m[3] || '').trim() : (attrs.value || '').trim();
      if (v) item(f[1]).comment = v;
    } else if ((f = /^completed_on-(ad[a-z0-9]{10})$/.exec(name))) {
      const v = (attrs.value || '').trim();
      let rec = records.find((r) => r.adId === f[1]);
      if (!rec) records.push((rec = { adId: f[1], completedOn: null }));
      if (v) rec.completedOn = v;
    } else if ((f = /^new-(ad[a-z0-9]{10})$/.exec(name))) {
      if (!records.some((r) => r.adId === f[1])) records.push({ adId: f[1], completedOn: null });
    }
  }
  return { items: Object.fromEntries(items), records };
}
...
module.exports = {
  ...
  classify, groupMeta, parseFragment, parseGridFragment, parseGridState, parseStandardState, scrubPersonal, containsYouthId,
};
