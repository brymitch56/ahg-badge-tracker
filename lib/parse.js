...
function parseStandardState(html, { awardId = null } = {}) {
  ...
  for (const f of fields) {
    ...
    if ((g = /^checkbox-([a-z0-9]{12})$/.exec(f.name)) && g[1] !== awardId) {
      // Live markup (verified 2026-09-13): a Krajee checkbox-x widget — an
      // <input type="text"> whose VALUE is "0"/"1" — not a real checkbox.
      // A real checkbox with a `checked` attribute is accepted too.
      item(g[1]).checked = 'checked' in f.attrs || /^(checked|true|1)$/i.test(f.attrs.checked || '')
        || ((f.attrs.type || 'text').toLowerCase() !== 'checkbox' && /^(1|true)$/i.test(f.value));
